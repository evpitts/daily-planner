...
with open(cover_data, 'wb') as f:
    f.write(bytearray('\\def\\author{' + author + '}\n', 'UTF-8'))
    f.write(bytearray('\\def\\firstDay{' + first_day.strftime('%-d %B %Y') + '}\n', 'UTF-8'))
    f.write(bytearray('\\def\\lastDay{' + last_day.strftime('%-d %B %Y') + '}\n', 'UTF-8'))
...
for d in dates:
    ...
    with open(file_name, 'wb') as f:
        f.write(bytearray('\\def\\date{' + date + '}\n', 'UTF-8'))
        f.write(bytearray('\\def\\dayOfWeek{' + day_of_week + '}\n', 'UTF-8'))
